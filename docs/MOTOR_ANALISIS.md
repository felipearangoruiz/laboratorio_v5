...
- **2026-04-23 — Sprint 5.C (capa Resultados en frontend). Sprint 5 completo.** Siete features sobre la capa Resultados: (i) badges de insights con conteo en nodos con findings; (ii) opacidad 0.5 para nodos que no figuran en `findings[*].node_ids ∪ recommendations[*].node_ids` del run; (iii) CTA "Ver en diagnóstico →" por-finding en `ResultsNodePanel` con deep-link al `NarrativePanel` (scroll + expand automático via `scrollIntoView`); (iv) navegación ant/sig entre nodos con findings (orden count desc + nombre asc, pulso breve del nuevo nodo en canvas); (v) render estructurado de `narrative_sections` con 5 secciones ordenadas (executive_summary → dimensions cards con score/std/barra/markdown/chips → transversal_findings → recommendations_summary → warnings con fondo amber), fallback silencioso a `narrative_md` monolítico para runs pre-5.A; (vi) coordinación bidireccional — chips de nodo hoverables + clickeables (navegan a `ResultsNodePanel`), headers de dimensión clickeables (switch a capa Análisis con `activeDimension`), `IntersectionObserver` adicional sobre dimension cards para soft-highlight al scroll; (vii) modo lectura ampliado (100% viewport, transición 0.25s, preserva scroll y estado). Sin refactor de layout — paneles `position:absolute` existentes. Sin cambios al backend ni al motor. Sprint 5 completo: backend contract (5.A) + capa Análisis (5.B) + capa Resultados (5.C). Producto listo para validación externa con pilot real.
- **2026-04-23 — Sprint 5.B (capa Análisis en frontend).** Cuatro features sobre el canvas con el contrato 5.A: (i) intensidad del borde de cada nodo proporcional al std heredado del bucket — mapeo [0, 0.5] → [1, 5] px (saturación ajustable vía `STD_SATURATION` en `OrgNode.tsx`); (ii) top-3 por tensión al clickear un score de dimensión (filtro superior o panel del nodo), desempate por std descendente — reemplaza el umbral fijo `tension > 40`; (iii) gris neutro explícito para nodos sin evidencia (ningún `dim.node_scores[nodeId]`) con tooltip accesible — cubre pragmáticamente los tres criterios del spec (sin entrada en scores / sin node_analysis / coverage<0.5 parcial); (iv) hover sobre fila de dimensión en el panel dispara pulse del ring en el top-3 (reutiliza `animate-ping`, sin CSS nuevo). Sin cambios al motor ni al backend. Fallback silencioso verificado para runs pre-5.A: scores vacío, nodos todos en gris, cero errores en consola. Próximo: Sprint 5.C — capa Resultados + panel narrativo expandido.
- **2026-04-23 — Sprint 5.A (contrato backend para capas Análisis y Resultados).** Output del Paso 4 gana `narrative_sections` con 5 claves (`executive_summary`, `dimensions`, `transversal_findings`, `recommendations_summary`, `warnings`), cada una con `markdown` + `node_ids` para trazabilidad al canvas. La sección `dimensions` incluye además `score` y `std` copiados de `org_analysis.dimension_scores`. `narrative_md` se mantiene por compatibilidad. Migración `20260423_0013` agrega columna JSONB nullable a `diagnosis_results`; runs anteriores a Sprint 5.A devuelven `null`. Backend: `submit_findings` ahora calcula y persiste el dict `scores` (antes `{}` vacío) con shape `{dim: {score, avg, std, node_scores, node_stds}}` — OPCIÓN 2 documentada: `node_stds[node_id]` hereda std del bucket (`parent_group_id`) al que pertenece el nodo. Esto habilita la modulación visual de bordes por dispersión en el canvas Análisis (Sprint 5.B). `EvidenceLinkCreate.finding_id` quedó como opcional desde Sprint 4.B Ronda 2; sin cambios. Próximo: Sprint 5.B — capa Análisis en frontend.
- **2026-04-23 — Sprint 4.B cerrado (Rondas 1-3).** Motor de análisis validado contra Meridian con tres dimensiones cubiertas: fundamentación (Ronda 1 — anti-alucinación en Paso 3, manejo de grupos `size=1` en Paso 2, fix del bug de convergencia en `_compute_confidence`), especificidad (Ronda 2 — `node_analyses_full` inyectado al Paso 4, reglas de estructura en findings, evidence_links obligatorias, recomendaciones ejecutables con lista negra de verbos vagos), y ajuste fino (Ronda 3 — findings individuales de grupos `size=1` permitidos, penalty single-node atenuado con evidencia verificada). Commit final de cierre amplía fuentes válidas de cita a `signals_tension` y `signals_positive`. Output validado: findings con cita textual verificada, evidence_links pobladas, recomendaciones con rol y plazo, confidence matemáticamente correcta. Limitaciones conocidas registradas en `DEUDA_DOCUMENTAL.md` (falso negativo del grupo Comercial bajo REGLA 6; dispersión de confidence limitada por la homogeneidad de Meridian), sujetas a validación con orgs reales en pilot externo. Próximo: Sprint 5 — capas Análisis y Resultados del canvas.
- **2026-04-23 — Sprint 4.B Ronda 2 (especificidad y evidence_links).** Reescritura del prompt Paso 4 con cinco reglas que fuerzan estructura de 3 oraciones (cuantificada + cita textual + consecuencia), evidence_links obligatorias con schema explícito (`node_analysis_id`, `group_analysis_id`, `quote`, `evidence_type`), `confidence_rationale` cuantitativo con al menos 3 factores, recomendaciones ejecutables con lista negra de verbos vagos y red de seguridad (prefiere lista corta a relleno), y prohibiciones de lenguaje en descripciones. `_run_paso4` inyecta ahora `node_analyses_full` al input_json (levanta la prohibición del prompt viejo de usar node_analyses, contradictoria con Sprint 4.A). Backend: `EvidenceLinkCreate.finding_id` pasa a opcional — el endpoint lo asigna server-side al crear el finding.
