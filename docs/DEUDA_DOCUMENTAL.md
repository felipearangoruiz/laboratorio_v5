...
| 11 | `PRD v2.1` (docx) | §7 — arquitectura de capas | Oficializar "3 capas" en el documento maestro de producto. Decisión tomada el 2026-04-21; pendiente bajar al PRD cuando el product owner actualice el archivo fuente. | Antes de cerrar Sprint 1 |
| 12 | `PRD v2.1` (docx) | §? — modelo de datos si aplica | Si el PRD describe explícitamente `Group` / `Member` / `LateralRelation` como entidades separadas, actualizar para reflejar el refactor Node+Edge. Verificación requiere pasada humana con el PRD abierto; Claude Code no puede parsear el .docx. | Antes de cerrar Sprint 1 |
