...
| Node | id, org_id, type (unit \| person), name, description, parent_node_id, position_x, position_y, attrs (jsonb) | Pertenece a Org. Puede tener children Nodes (jerarquía via parent_node_id). Puede ser source o target de Edges. Tiene NodeStates por cada Campaign en la que participa. |
| Edge | id, source_node_id, target_node_id, edge_type (lateral \| process), edge_metadata (jsonb), created_at | Conecta dos Nodes. La jerarquía NO se modela como Edge: vive en Node.parent_node_id. |
...
| NodeState | id, node_id, campaign_id, status, invited_at, completed_at, interview_data (jsonb), respondent_token | Estado de captura de un Node en el contexto de una Campaign específica. Único por (node_id, campaign_id). |
