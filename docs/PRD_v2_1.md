...
| AssessmentCampaign | id, organization_id, created_by_user_id (nullable), name, status (draft \| active \| closed), started_at, closed_at | Pertenece a Organization. Agrupa todos los NodeStates, Documents y Diagnostics de un evento de diagnóstico. `closed_at` alinea con el estado terminal `status='closed'`; `created_by_user_id` audita quién lanzó cada diagnóstico. |
