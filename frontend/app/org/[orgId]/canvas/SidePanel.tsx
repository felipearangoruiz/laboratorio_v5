...
export default function SidePanel({
  ...
  async function handleSave() {
    setSaving(true);
    if (isPerson) {
      await onUpdate(nodeId, { name, tarea_general: role, email });
    } else {
      await onUpdate(nodeId, { name, description, area });
    }
    setSaving(false);
    onClose();
  }
  ...
  async function handleAddMember() {
    ...
    try {
      ...
      if (res.ok) {
        setNewMemberName(""); setNewMemberRole(""); setNewMemberEmail("");
        await loadMembers();
      }
    } catch { /* ignore */ }
    ...
    <div className="absolute top-0 right-0 h-full w-80 bg-warm-50 border-l border-warm-200 shadow-warm-md z-10 flex flex-col">
            ...
            {isPerson ? (
              <>
                <div>
                  ...
                  <input type="text" value={role} onChange={(e) => setRole(e.target.value)} className={inputCls} placeholder="Ej: Director de Tecnología" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-warm-900 mb-1.5">Email</label>
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className={inputCls} placeholder="Ej: juan@empresa.com" />
                </div>
              </>
            ) : (
              <>
                <div>
                  ...
                  <input type="text" value={area} onChange={(e) => setArea(e.target.value)} className={inputCls} placeholder="Ej: Tecnología" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-warm-900 mb-1.5">Descripción</label>
                  <textarea value={description} onChange={(e) => setDescription(e.target.value)} className={`${inputCls} resize-none`} rows={3} placeholder="Descripción opcional del área..." />
                </div>
              </>
            )}
      ...
      {tab === "members" && !isPerson && (
        <>
              ...
              <div className="space-y-2">
                {members.map((m) => (
                  <div key={m.id} className="flex items-center justify-between px-3 py-2.5 bg-white rounded-md border border-warm-200">
                    ...
                    <button onClick={() => handleRemoveMember(m.id)} className="text-warm-300 hover:text-red-500 flex-shrink-0 ml-2 transition-colors">
          ...
          <div className="px-5 py-4 border-t border-warm-200 bg-white space-y-2">
            <input type="text" value={newMemberName} onChange={(e) => setNewMemberName(e.target.value)} className={inputCls} placeholder="Nombre" />
            <input type="text" value={newMemberRole} onChange={(e) => setNewMemberRole(e.target.value)} className={inputCls} placeholder="Cargo (opcional)" />
            <button
